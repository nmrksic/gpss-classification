function plot_additive_decomp( X, y, post, kernel_components, componentDims, kernel_params, show_samples, savefigs, fileprefix, dimensionLabels )
%
...
for i = 1:num_components
  ...
  %  L = chol(complete_sigma);  % L will come from approximate inference alg
    ...
    if num_dims == 1
        nice_oned_plot( X(:,cur_ds), y, data_mean, xrange, component_mean, component_var, show_samples, savefigs, filename)
    elseif num_dims == 2
        nice_twod_plot( a,b,X(:,cur_ds), y, data_mean, xstar(:, cur_ds), component_mean, component_var, savefigs, filename)
    end
...
function nice_oned_plot( X, y, y_adjusted, xstar, mean, variance, show_samples, savefigs, filename)
...
function nice_twod_plot( a,b,X, y, y_adjusted, xstar, mean, variance, savefigs, filename)
