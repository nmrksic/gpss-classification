function evaluateSyntheticNoSP ( numRestarts, runParallel)
    ...
    encoders{1} = [2 0 0; 2 0 0; 2 0 0];            dims{1} = 3;     % SE2 in 3D
 ...
 %   encoders{5} = [3 5 7 9; 1 0 0 0; 10 0 0 0];    dims{5} = 10;   % SE1 + SE3 X SE5 X SE7 X SE9 + SE10, 10D
   ...
   % covParams{3} = [1 0 0 0];
    ...
    n = 1000; % we generate this data, then subsample it to get 100, 300 data points.
    ...
    sampler0 = randsample(1:1000, 700);
    sampler01 = randsample(1:700, 500);
    ...
    for i = 1:length(covFunction)
        ...
        for SNR = experimentsSNR
            ...
            for sample = [1000, 700, 500, 300, 100]
                ...
                elseif sample == 700
                    ...
                    X_trn = X_trn(sampler0, :);
                    y_trn = y_trn(sampler0, :);
                    X_tst = X_tst(sampler0, :);
                    y_tst = y_tst(sampler0, :);
                
                elseif sample == 500
                    
                    X_trn = X_trn(sampler01, :);
                    y_trn = y_trn(sampler01, :);
                    X_tst = X_tst(sampler01, :);
                    y_tst = y_tst(sampler01, :);
            ...
            if SNR == 100
                currentLine = currentLine - 5; % go three lines back to write in 1
            end
    ...
    results2  = cell(experimentCount * 5, 9);
